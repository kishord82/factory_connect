  ...
  try {
    ...
    const result = await withTenantClient(ctx, async (client: PoolClient) => {
      const [ordersByStatus, shipmentsByStatus, topBuyers, revenueByMonth] = await Promise.all([
        client.query(
          `SELECT status, COUNT(*)::int as count FROM orders.canonical_orders GROUP BY status ORDER BY count DESC`,
        ),
        client.query(
          `SELECT status, COUNT(*)::int as count FROM orders.canonical_shipments GROUP BY status ORDER BY count DESC`,
        ),
        client.query(
          `SELECT b.name, COUNT(*)::int as total_orders
           FROM orders.canonical_orders o
           JOIN core.connections c ON o.connection_id = c.id
           JOIN core.buyers b ON c.buyer_id = b.id
  ...
  try {
    ...
    const stats = await withTenantClient(ctx, async (client: PoolClient) => {
      const [orders, shipments, invoices, connections] = await Promise.all([
        client.query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = \'DRAFT\') as draft, COUNT(*) FILTER (WHERE status = \'CONFIRMED\') as confirmed FROM orders.canonical_orders'),
        client.query('SELECT COUNT(*) as total FROM orders.canonical_shipments'),
        client.query('SELECT COUNT(*) as total FROM orders.canonical_invoices'),
        client.query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = \'active\') as active FROM core.connections'),
      ]);
      return {
...
analyticsRouter.get('/order-volume', validate({ query: DateRangeQuery }), async (req, res, next) => {
  try {
    ...
    const result = await withTenantClient(ctx, async (client: PoolClient) => {
      ...
      let sql = `SELECT DATE(created_at) as date, COUNT(*) as count FROM orders.canonical_orders WHERE 1=1`;
      let idx = 1;
    ...
    const result = await withTenantClient(ctx, async (client: PoolClient) => {
      const r = await client.query(
        `SELECT current_step, COUNT(*) as count,
         ...
         FROM workflow.order_sagas GROUP BY current_step ORDER BY count DESC`,
      );
      return r.rows;
    });
    res.json({ data: result });
...
analyticsRouter.get('/escalations', validate({ query: DateRangeQuery }), async (req, res, next) => {
  try {
    ...
    const result = await withTenantClient(ctx, async (client: PoolClient) => {
      const params: unknown[] = [];
      let sql = `SELECT trigger_type, COUNT(*) as count, MAX(created_at) as latest FROM platform.escalation_log WHERE 1=1`;
      let idx = 1;
      if (q.from) { sql += ` AND created_at >= $${idx++}`; params.push(q.from); }
      if (q.to) { sql += ` AND created_at <= $${idx++}`; params.push(q.to); }
