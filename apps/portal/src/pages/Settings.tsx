...
type ActiveTab = 'users' | 'flags' | 'config' | 'notifications' | 'audit';
...
function UsersTab({ user, queryClient }: UsersTabProps): React.ReactElement {
...
function NotificationsTab({ queryClient }: NotificationsTabProps): React.ReactElement {
...
function FlagsTab({ user, queryClient }: FlagsTabProps): React.ReactElement {
...
function AuditTab(): React.ReactElement {
  ...
  const auditQuery = useQuery({
    ...
    queryFn: () =>
      api.get<{ data: AuditLog[]; total: number; totalPages: number }>(
...
export function Settings(): React.ReactElement {
  ...
  const tabs: Array<{ id: ActiveTab; label: string; icon: string }> = [
    { id: 'users', label: `Users (${userCount})`, icon: '👤' },
    { id: 'notifications', label: 'Notifications', icon: '🔔' },
    { id: 'config', label: 'Configuration', icon: '⚙️' },
    ...(isAdmin(user)
      ? [
          { id: 'flags' as const, label: `Feature Flags (${flagCount})`, icon: '🚩' },
  ...
  return (
    <div>
      ...
      {activeTab === 'users' && <UsersTab user={user} queryClient={queryClient} />}
      {activeTab === 'notifications' && <NotificationsTab queryClient={queryClient} />}
      {activeTab === 'config' && <ConfigTab />}
      {activeTab === 'flags' && isAdmin(user) && <FlagsTab user={user} queryClient={queryClient} />}
      {activeTab === 'audit' && isAdmin(user) && <AuditTab />}
    </div>
  );
}
