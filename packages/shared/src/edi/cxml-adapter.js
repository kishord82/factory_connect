 ...
 */
function extractXmlText(tag, content) {
    const match = content.match(new RegExp(`<${tag}[^>]*>([^<]*)</${tag}>`));
    return match?.[1] ?? '';
}
function extractXmlAttr(tag, attr, content) {
    const match = content.match(new RegExp(`<${tag}[^>]*${attr}="([^"]*)"[^>]*>`));
    return match?.[1] ?? '';
}
function extractXmlBlock(tag, content) {
 ...
 */
export function parseCxmlOrderRequest(xml) {
    try {
        const data = {
 ...
 */
export function buildCxmlEnvelope(content, config) {
...
//# sourceMappingURL=cxml-adapter.js.map