 ...
 */
export function padField(val, len, char = ' ', direction = 'right') {
 ...
 */
export function padFieldNum(val, len) {
 ...
 */
export function formatEdiDate(date, includeYear = false) {
 ...
 */
export function formatEdiTime(date, includeSeconds = false) {
    const d = date instanceof Date ? date : new Date(date);
 ...
 */
function generateGS(docType, senderId, receiverId, controlNumber) {
    const now = new Date();
    const codeMap = { '855': 'PR', '856': 'SH', '810': 'IN', '997': 'FA' };
    return [
 ...
 */
function generateGE(controlNumber) {
 ...
 */
export function nextControlNumber(_type, _config) {
 ...
 */
export function generateEnvelope(docType, config, content, segmentCount) {
...
//# sourceMappingURL=x12-generator.js.map