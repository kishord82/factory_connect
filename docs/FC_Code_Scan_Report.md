...
**Total source:** ~44,687 lines across 180 source files + 47 test files
...
|-----------|-------|--------|
| TypeScript safety | 55/100 | 🔴 Critical — API fails tsc with 23 errors |
| ESLint compliance | 52/100 | 🔴 Critical — 818 errors, 141 warnings across packages |
| Security posture | 78/100 | 🟡 Major — 2 CVEs in vite, `as any` in sensitive services |
| Test health | 62/100 | 🟡 Major — 13 tests failing, 0% coverage instrumentation |
| Code standards | 60/100 | 🟡 Major — SELECT *, console.log, TODO stubs |
| Dead code | 88/100 | 🟢 Minor — 4 unused exports found |
| **Overall** | **66/100** | 🟡 Not production-ready |

**Critical path to production:** Fix the TypeScript errors (missing `@fc/observability` types + missing constant exports), then address `SELECT *` in production SQL and the `as any` casts in reconciliation/notice services.
...
*Generated by automated code scan on 2026-04-16. Run `pnpm lint && pnpm typecheck` to verify fixes.*
